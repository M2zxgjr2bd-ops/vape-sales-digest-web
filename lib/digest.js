...
function aiExcerpt(item, maxChars) {
  const raw = item.articleText || item.articleDescription || "";
  if (!raw) {
    return "正文抓取不足，待人工核验。";
  }
...
function renderFallbackDigest({ items, runAt, timeZone, openAiConfigured = false }) {
  ...
  lines.push(
    openAiConfigured
      ? "- 当前显示的是基础回退版摘要；本次 AI 总结未成功完成，建议检查超时设置或继续缩小输入。"
      : "- 当前是基础回退版摘要；配置 OPENAI_API_KEY 后，日报会更贴近销售判断。"
  );

  return lines.join("\n");
}

async function renderAiDigest({
  items,
  runAt,
  timeZone,
  openAiApiKey,
  ...
  const date = formatDateParts(runAt, timeZone);
  const itemLines = items.slice(0, openAiItemLimit).map((item, index) => {
    return [
      ...
      `article_excerpt: ${aiExcerpt(item, openAiExcerptChars)}`
    ].join("\n");
  });
  ...
  if (!markdown) {
    throw new Error("OpenAI returned empty digest text");
  }
...
export async function buildDigest({
  ...
  return {
    markdown: renderFallbackDigest({ items, runAt, timeZone, openAiConfigured: false }),
