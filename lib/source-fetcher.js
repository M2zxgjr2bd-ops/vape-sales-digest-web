...
function scoreCandidate({ text, href, keywords, source }) {
  ...
  if (/\/search\/|[?&](page|s|q)=/i.test(href)) {
    score -= 4;
  }

  if (/guidance and regulation|news and communications|collections|best disposable|best pod|top 10/i.test(text)) {
    score -= 3;
  }

  if (/best|review|guide/i.test(text) && source.group !== "regulatory") {
    score -= 2;
  }
